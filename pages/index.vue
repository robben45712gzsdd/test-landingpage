<template>
  <div class="relative bg-black w-full overflow-x-hidden">
    <!-- Animated Background -->
    <div class="fixed inset-0 bg-gradient-to-br from-red-950 via-black to-purple-950 opacity-40 pointer-events-none">
    </div>
    ...
    <header ref="headerNav" :class="[
      ...
      <nav class="relative flex justify-between items-center mx-auto px-12 h-full">
        <!-- Logo -->
        <div ref="headerLogo" class="group relative transition-all duration-700"
        ...
        <!-- Center Slogan -->
        <div ref="headerSlogan" class="left-1/2 absolute transition-all -translate-x-1/2 duration-700"
          :class="currentSection === 'home' ? 'opacity-0 scale-90' : 'opacity-100 scale-100'">
          <p
            class="hidden md:!block bg-clip-text bg-gradient-to-r from-red-500 via-pink-500 to-purple-500 font-bold text-transparent text-base lg:text-lg tracking-wide whitespace-nowrap">
        ...
        <div v-show="currentSection !== 'home'" class="hidden md:flex gap-8 transition-all duration-500 ease-in-out"
          ...
          <a v-for="section in ['home', 'games', 'about']" :key="section" :href="`#${section}`" :class="[
            ...
            {{ section }}
            <span v-if="currentSection === section"
              class="right-0 -bottom-2 left-0 absolute bg-gradient-to-r from-red-500 to-pink-500 shadow-[0_0_10px_rgba(239,68,68,0.8)] h-0.5"></span>
          </a>
        </div>
      ...
      <div v-if="mobileMenuOpen" class="md:hidden bg-black/95 backdrop-blur-2xl border-red-500/20 border-t">
        <a v-for="section in ['home', 'games', 'about']" :key="section" :href="`#${section}`"
    ...
    <!-- Section Dots -->
    <div class="hidden top-1/2 right-6 lg:right-10 z-[1800] fixed lg:flex flex-col gap-5 -translate-y-1/2">
      <button v-for="(dot, index) in sectionDots" :key="index" :class="[
      ...
      ]" :title="dot.label" @click="scrollToSection(dot.id)"></button>
    </div>
    ...
    <!-- Loading State -->
    <div v-if="loading" class="z-[3000] fixed inset-0 flex justify-center items-center bg-black">
      <div class="flex flex-col justify-center items-center text-center">
        <div class="relative mb-6">
          <img src="@/assets/logo/5_white.png" alt="NEKSTUDIO"
            class="opacity-80 drop-shadow-[0_0_20px_rgba(239,68,68,0.5)] w-auto h-16 md:h-20 object-contain animate-breathing" />
        </div>
        <p
          class="bg-clip-text bg-gradient-to-r from-red-500 via-pink-500 to-purple-500 font-bold text-transparent text-lg md:text-xl tracking-wide animate-breathing animate-gradient">
          True Game - Real Entertainment
        </p>
      </div>
    </div>

    <div v-if="error && !loading" class="z-[3000] fixed inset-0 flex justify-center items-center bg-black px-4">
      <div class="max-w-md text-center">
        <div class="relative mx-auto mb-6 w-20 h-20">
          <div class="absolute inset-0 bg-red-500/20 rounded-full animate-ping"></div>
          <svg class="relative w-20 h-20 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <section id="home" ref="heroSection"
      ...
      <div class="z-10 relative px-4 md:px-8 max-w-5xl text-center">
        <div ref="heroTitle" class="mb-6 overflow-hidden">
          <h1 class="mb-2 font-black text-5xl lg:text-7xl md:text-8xl leading-none tracking-tight">
            <span
              class="bg-clip-text bg-gradient-to-r from-red-500 via-pink-500 to-red-600 text-transparent text-4xl md:text-6xl animate-gradient">
              TRUE GAME
            </span>
          </h1>
          <h1 class="font-black text-5xl lg:text-7xl md:text-8xl leading-none tracking-tight">
            <span
              class="bg-clip-text bg-gradient-to-r from-pink-500 via-purple-500 to-red-500 text-transparent text-3xl md:text-6xl animate-gradient animation-delay-300">
              REAL ENTERTAINMENT
            </span>
          </h1>
        </div>
        ...
        <p ref="heroSubtitle"
          class="mx-auto mb-10 max-w-3xl font-light text-white/90 text-lg md:text-2xl leading-relaxed">
          Experience next-generation gaming with stunning visuals, instant
          gameplay, and revolutionary design
        </p>

        <button ref="heroButton" @click="scrollToSection('games')"
    ...
    <!-- GAMES SECTION -->
    <section v-if="!loading && !error" id="games" ref="gamesSection" class="relative bg-black pb-20 md:pb-0 w-full">
      <!-- Transition Blend Overlay -->
      <div ref="transitionBlend"
      ...
      <div ref="gameVideosContainer"
        class="hidden md:block top-0 right-0 left-0 z-[1] fixed h-screen overflow-hidden pointer-events-none">
        <!-- Hero Video (index -1) -->
        <video ref="heroVideoFixed"
          class="absolute inset-0 !brightness-[0.4] w-full h-full object-cover transition-opacity duration-1000" :class="{
        ...
        <!-- Game Videos -->
        <video v-for="(game, index) in games" :key="game.id" :ref="`gameVideo${index}`"
          class="absolute inset-0 brightness-[1.1] w-full h-full object-cover transition-opacity duration-1000" :class="{
            'opacity-100': currentGameIndex === index,
            'opacity-0': currentGameIndex !== index,
          }" :src="game.videoUrl" muted playsinline autoplay loop></video>

        <!-- Split Overlay: Left side dark, Right side clear -->
        <div class="absolute inset-0 pointer-events-none">
          <!-- Hero Overlay (when currentGameIndex === -1) - Very light overlay -->
          <div v-if="currentGameIndex === -1"
          ...
          <template v-else>
            ...
            <div class="absolute inset-0 transition-opacity duration-1000" :style="{
              background: `radial-gradient(circle at ${70 + currentGameIndex * 5
      ...
      <div class="z-10 relative">
        ...
        <div v-for="(game, index) in games" :key="game.id" class="w-full md:h-screen" :ref="`gameSection${index}`"
          ...
          <div class="md:hidden px-4 py-6">
            <div
              ...
              <div class="relative">
                <video class="shadow-2xl mb-5 rounded-2xl w-full h-52 object-cover" :src="game.videoUrl" muted
                  playsinline autoplay loop></video>

                <div class="space-y-4">
                  <div class="space-y-1">
                    <h3 class="drop-shadow-[0_0_25px_rgba(239,68,68,0.8)] font-black text-red-500 text-3xl">
                      {{ game.title }}
                    </h3>
                  </div>
          ...
          <div v-show="currentGameIndex === index"
            ...
            <div class="space-y-8">
              <div class="flex">
                <img :src="game.linkImg" :alt="`${game.title} Logo`"
                  class="drop-shadow-[0_0_25px_rgba(239,68,68,0.6)] max-w-[280px] max-h-34 object-contain" />
              </div>

              <p class="pr-24 font-bold text-white/90 text-xl lg:text-2xl leading-relaxed">
                {{ game.description }}
              </p>
              ...
              <div v-else class="flex gap-6">
                ...
                <a href="https://play.google.com/store" target="_blank"
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" xmlns:cc="http://creativecommons.org/ns#"
                    ...
                    id="svg51" width="150" height="48" version="1.1" viewBox="0 0 180 53.333" xml:space="preserve"
                    class="w-auto h-12">
                    ...
                    <path id="path33"
                      ...
                      fill="#fff" stroke="#fff" stroke-miterlimit="10" stroke-width=".26666" />
                  </svg>
                </a>
              </div>
            </div>
    ...
    <section id="about" ref="aboutSection"
      ...
      <div class="relative mx-auto max-w-4xl">
        <div class="absolute -inset-20 bg-red-500/10 opacity-30 blur-3xl rounded-full"></div>
        ...
        <div class="relative space-y-6 md:space-y-8 font-light text-white/85 text-lg md:text-xl leading-relaxed">
          <p
            ...
            Nek Studio is a mobile game startup in Vietnam. Originally a team specializing in NFTs (Non-fungible Token),
            we
            aim to create products that offer long-term entertainment, prioritizing core values ​​over time rather than
            chasing trends.
          </p>
          <p
            class="bg-white/5 hover:bg-white/10 backdrop-blur-sm p-6 border border-red-500/20 hover:border-red-500/40 rounded-2xl transition-all duration-300">
    ...
    <!-- FOOTER -->
    <footer class="z-20 relative bg-black/90 px-6 md:px-12 py-12 md:py-16 border-red-500/20 border-t w-full">
      <div class="mx-auto max-w-6xl">
        ...
        <div class="gap-6 md:gap-8 grid grid-cols-1 md:grid-cols-3 mb-10 md:mb-12">
          ...
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <a href="tel:+84865618705" class="text-white/60 hover:text-white text-sm transition-colors">
                +84 865-618-705
              </a>
            </div>
          </div>
          ...
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <a href="mailto:Admin@neksolution.com"
        ...
        <!-- Divider -->
        <div class="bg-gradient-to-r from-transparent via-red-500/30 to-transparent mb-6 rounded-full w-full h-px">
        </div>
        ...
        <div class="flex md:flex-row flex-col justify-between items-center gap-4 text-center">
          ...
          <!-- <div class="flex flex-wrap justify-center gap-4 md:gap-6 text-xs md:text-sm">
            <nuxt-link to="/privacy-policy" class="text-white/50 hover:text-red-400 transition-colors">
              Privacy Policy
            </nuxt-link>
            <a href="#" class="text-white/50 hover:text-red-400 transition-colors">
              Terms of Service
            </a>
          </div> -->
        </div>
